...
RUN pip3 install --no-cache-dir --break-system-packages "setuptools>=63" "numpy<2" "pandas<3" "matplotlib<4" "scipy<2" \
...
RUN cd omnetpp \
    ...
    && find doc -mindepth 1 -maxdepth 1 ! -name etc -exec rm -rf {} +   # doc/etc contient les DTD requises
